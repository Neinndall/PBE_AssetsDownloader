    ...
    public partial class JsonDiffControl : UserControl
    {
        ...
        public async Task LoadAndDisplayDiffAsync(string oldJson, string newJson, string oldFileName, string newFileName)
        {
            try
            {
                ...
                {
                    _customMessageBoxService.ShowInfo("Comparison Result", "No differences found. The two files are identical.", Window.GetWindow(this));
                }
            }
            catch (Exception ex)
            {
                _customMessageBoxService.ShowError("Error", $"Failed to load comparison: {ex.Message}. Check logs for details.", Window.GetWindow(this));
            }
        }
